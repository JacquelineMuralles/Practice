...
// addOne(NaN)                  // NaN
function addOne(x) {
    //giant hint for Monday(know how to parseInt or parseFloat)
    if(!isNaN(x)){
        return x + 1
    }else {
        return (Number(x)) + 1
...
function isEven(input) {
    //got wrong because I had || instead of &&
...
function isIdentical(ObjA, ObjB) {
    return ObjA === ObjB;
}
...
function concat(input1, input2) {
    if (typeof (input1 || input2) === ""){
        return input1 + input2 + "";
    }else if (!isNaN(input1 || input2)) {
        return (!isNaN(input1) || !isNaN(input2)) + (!isNaN(input1) || !isNaN(input2)) + ""
    }
}
//answer
//function concat(input1, input2) {
